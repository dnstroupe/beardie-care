...
const WEEK_DATA = [
  {
    day:"Monday",
    feeders:"Dubia Roaches",
    qty:"8–12 roaches",
    salad:"Collard + Mustard Greens + Squash",
    saladAmount:"¼–½ cup chopped · offer at morning lights-on · remove after 2–3 hrs",
    supplement:"Calcium Plus LoD",
    suppHow:"Lightly dust live feeders · shake in small bag to coat",
    hydration:"Fresh water — refill dish",
  ...
  },
  {
    ...
    suppHow:"Sprinkle lightly over salad · do not dust feeders",
    hydration:"Fresh water — refill dish",
    task:"Wipe glass / check poop"
  },
  {
    day:"Wednesday",
    feeders:"Calcigrubs / Nutrigrubs",
    qty:"15–20 grubs",
  ...
  },
  {
    ...
    supplement:"Calcium Plus LoD",
    suppHow:"Lightly dust live feeders · shake in small bag to coat",
    hydration:"Extra hydration — offer hornworm for moisture",
...
const ROW_FIELDS = [
  { key:"feeders",     label:"Live Feeders" },
  { key:"salad",       label:"Salad Mix" },
  { key:"saladAmount", label:"Amount & Timing" },
  { key:"supplement",  label:"Supplement" },
  { key:"suppHow",     label:"How to Apply" },
  { key:"hydration",   label:"Hydration" },
  { key:"task",        label:"Care Task" }
];

const TIPS = [
  "💡 Consistency today = a happy Yoohoo tomorrow! 🦎",
  "☀️ Always verify basking temps first — Yoohoo needs 100–105°F to thrive.",
  "❄️ Cool side should stay 75–82°F — check it daily.",
  "💧 Soak Yoohoo 2–3× per week — great for hydration and shedding!",
  "🥗 Variety in greens = better nutrition — rotate daily!",
  "🪲 Remove uneaten insects within 15 minutes — they can bite.",
  "⚖️ Weigh Yoohoo monthly — she started at 80g, watch her grow!",
  "☀️ UVB bulbs lose effectiveness before they burn out — replace every 6 months.",
  "🦎 A basking Yoohoo is a happy Yoohoo — that is her throne!",
  "💊 Don't skip supplements — they prevent metabolic bone disease.",
  "🌡️ Wrong temps are the #1 cause of illness in bearded dragons.",
  "❤️ Yoohoo knows your voice — she is curious about everything!",
  "🐛 Hornworms are high in water content — great for hydration days!",
  "🪳 Dubia roaches are Yoohoos main protein — buy ¼ inch size at 80g.",
  "🧗 Yoohoo loves to climb — make sure her enclosure has plenty of branches!",
  "🌟 Yoohoo is Yuzuriha — explorer, adventurer, queen of her domain."
];

// Helpers
function todayIdx() { const d = new Date().getDay(); return d === 0 ? 6 : d - 1; }
function sk(k) { return "yoohoo_" + k; }
...
function setDates() {
  ...
  if (titleEl) titleEl.textContent = `${days[now.getDay()]}'s Care — Yoohoo`;
...
function buildTodayCard() {
  ...
  grid.innerHTML = `
    ...
    <div class="today-cell ice-cell" style="grid-column: span 2">
      <span class="today-cell-label">⏱ Salad Amount & Timing</span>
      <span class="today-cell-value" style="font-size:0.82rem">${t.saladAmount}</span>
    </div>
    <div class="today-cell">
    ...
    <div class="today-cell ice-cell">
      <span class="today-cell-label">📋 How to Apply</span>
      <span class="today-cell-value" style="font-size:0.82rem">${t.suppHow}</span>
    </div>
    <div class="today-cell ice-cell">
...
function buildWeekTable() {
  ...
  ROW_FIELDS.forEach(function(field) {
    ...
    // Detail rows (no checkbox needed — informational only)
    const detailOnly = field.key === "saladAmount" || field.key === "suppHow";
    tr.className = detailOnly ? "data-row detail-row" : "data-row";

    WEEK_DATA.forEach(function(d, i) {
      const td = document.createElement("td");
      if (i === today) td.style.background = detailOnly ? "rgba(93,174,238,0.04)" : "rgba(230,57,37,0.04)";

      if (detailOnly) {
        // Detail row — no checkbox, just info text
        td.innerHTML = `
          <span class="cell-label detail-label">${field.label}</span>
          <span class="cell-detail-val">${d[field.key]}</span>`;
      } else {
        ...
        td.innerHTML = `
          ...
          <label class="cell-check ${isDone ? "row-done" : ""}" data-key="${rowKey}">
            ...
            <span class="cell-val">${field.key === "feeders" ? d[field.key] + " · " + d.qty : d[field.key]}</span>
          </label>`;
...
function updateWeekProgress() {
  const checkableFields = ROW_FIELDS.filter(function(f) { return f.key !== "saladAmount" && f.key !== "suppHow"; });
  WEEK_DATA.forEach(function(_, i) {
    const td = document.getElementById("prog" + i);
    if (!td) return;
    let done = 0;
    checkableFields.forEach(function(f) { if (load("w" + i + "_" + f.key)) done++; });
    td.textContent = "Completed " + done + " / " + checkableFields.length;
  });
}
...
function initResetBtn() {
  ...
  if (!btn) return;
  btn.addEventListener("click", function() {
    if (!confirm("Reset all weekly checkboxes for Yuzu?")) return;
    const checkable = ROW_FIELDS.filter(function(f) { return f.key !== "saladAmount" && f.key !== "suppHow"; });
    WEEK_DATA.forEach(function(_, i) {
      checkable.forEach(function(f) { save("w" + i + "_" + f.key, false); });
    });
...
function renderWeightTable() {
  ...
  tbody.innerHTML = "";
  if (!entries.length) {
    tbody.innerHTML = '<tr><td colspan="4" class="no-entries">No entries yet — add Yoohoo\'s first weight above! She started at 80g</td></tr>';
    return;
  }
...
// ── LEGEND COLLAPSIBLE ────────────────────
function initLegend() {
  const btn  = document.querySelector('.legend-trigger');
  const body = document.getElementById('legendBody');
  const arr  = document.getElementById('legendArr');
  if (!btn || !body) return;

  // Default: open on first visit, remember after
  const saved = localStorage.getItem('col_legendBody');
  if (saved === '1') {
    body.classList.add('closed');
    if (arr) arr.classList.add('up');
  }
  ...
  btn.addEventListener('click', function() {
    ...
    try { localStorage.setItem('col_legendBody', closing ? '1' : '0'); } catch(e) {}
  });
}
...
function printToday() {
  ...
  sheet.innerHTML = `
    ...
    <div class="print-detail"><span>Never leave insects in enclosure overnight</span>They can bite and stress Yoohoo</div>
    <br><p style="text-align:center;color:#D4870A;font-weight:bold;">🦎 Yuzuriha Yoohoo — Explorer — Adventurer — Dragon</p>
...
function shareMorningSalad() {
  ...
  // Determine supplement instruction
  const suppOnSalad = t.suppHow.toLowerCase().includes("salad");
  const suppLine = suppOnSalad
    ? `💊 ${t.supplement} — sprinkle lightly on top of salad`
    : `💊 No supplement on salad today`;

  // Build the card text for copying
  const cardText = `🥗 Yuzu's Morning Salad — ${dateStr}

Greens: ${t.salad}
Amount: ${t.saladAmount.split('·')[0].trim()}

${suppLine}

☀️ Offer salad after lights on and temps are up
⏱ Remove after 2–3 hours
❄️ Never use yesterday's leftovers

💡 Quick check — verify basking temp once mid-morning!
  ...
  preview.innerHTML = `
    ...
    <div class="sc-row">
      ...
      <div>
        ...
        <div class="sc-val">${t.salad}</div>
      </div>
    </div>
    <div class="sc-row">
      <span class="sc-icon">⏱</span>
      <div>
        <div class="sc-label">Amount & Timing</div>
        <div class="sc-val">${t.saladAmount.split('·')[0].trim()} · remove after 2–3 hrs</div>
      </div>
    </div>
    <div class="sc-row ${suppOnSalad ? 'sc-supp-on' : 'sc-supp-off'}">
      <span class="sc-icon">💊</span>
      <div>
        <div class="sc-label">Supplement</div>
        <div class="sc-val">${suppOnSalad ? t.supplement + ' — sprinkle lightly on top' : 'No supplement on salad today'}</div>
      </div>
    </div>
    <div class="sc-row">
      <span class="sc-icon">❄️</span>
      <div>
        <div class="sc-label">Important</div>
        <div class="sc-val">Never use yesterday's leftovers</div>
      </div>
    </div>
    <div class="sc-tip">💡 Quick check — verify basking temp once mid-morning!</div>
  `;

  // Store card text for copying
  copyBtn.dataset.text = cardText;
...
function copyCard() {
  ...
  if (navigator.clipboard) {
    navigator.clipboard.writeText(text).then(function() {
      ...
      setTimeout(function() { btn.textContent = "📋 Copy Card"; }, 2500);
    });
  } else {
    // Fallback
