...
const TIPS = [
  "💡 Consistency today = a healthy Yuzu tomorrow.",
  "🔥 Always verify basking temps before adjusting Yuzu's diet.",
  "❄️ Cool side should stay 75–82°F — check it daily.",
  "💧 Soaking 2–3× per week keeps Yuzu hydrated and helps shedding.",
  "🥗 Variety in greens = better nutrition — rotate daily!",
  "🪲 Remove uneaten insects within 15 minutes — they can bite.",
  "⚖️ Weigh Yuzu monthly — steady weight gain means a healthy dragon.",
  "☀️ UVB bulbs lose effectiveness before they burn out — replace every 6 months.",
  "🦎 A basking Yuzu is a happy dragon — perfectly normal behavior!",
  "💊 Don't skip supplements — they prevent metabolic bone disease.",
  "🌡️ Wrong temps are the #1 cause of illness in bearded dragons.",
  "❤️ Yuzu knows your voice — talk to her during feeding time.",
  "🐛 Hornworms are high in water — refrigerate to slow growth. Use within the week.",
  "🪳 Dubia roaches are the gold standard feeder — buy ¼ inch size at 80g."
];

// Helpers
function todayIdx() { const d = new Date().getDay(); return d === 0 ? 6 : d - 1; }
function sk(k) { return "yuzu_" + k; }
...
function setDates() {
  ...
  if (titleEl) titleEl.textContent = `${days[now.getDay()]}'s Care — Yuzu`;
...
function renderWeightTable() {
  ...
  tbody.innerHTML = "";
  if (!entries.length) {
    tbody.innerHTML = '<tr><td colspan="4" class="no-entries">No entries yet — add Yuzu\'s first weight above!</td></tr>';
    return;
  }
...
function printToday() {
  ...
  sheet.innerHTML = `
    ...
    <h1>🔥 Yuzu's Daily Care Sheet ❄️</h1>
    <p>${dateStr}</p>
    ...
    <div class="print-detail"><span>Never leave insects in enclosure overnight</span>They can bite and stress Yuzu</div>
    <br><p style="text-align:center;color:#C96A3A;font-weight:bold;">🔥 Yuzu — Half Fire. Half Ice. ❄️</p>
